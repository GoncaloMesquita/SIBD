...
try:
    ...
    if len(result) == 0:
        print("Trip doesn't exist")
    else:
        delete_trip = "DELETE FROM trip WHERE takeoff = %(takeoff)s AND reservation_start_date = %(r_start_date)s AND reservation_end_date = %(r_end_date)s  AND boat_country = %(boat_country)s AND cni = %(cni)s"
        cursor.execute(delete_trip, {'takeoff': takeoff, 'r_start_date': r_start_date
                                    , 'r_end_date': r_end_date, 'boat_country': boat_country, 'cni': cni})
        connection.commit()
        print('Trip deleted: SUCCESS')
    #End Connection
    cursor.close()
    connection.close()
